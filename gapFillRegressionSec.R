...
model.data = read.csv(paste(mainDir, "data/gapFill/Filled_stations_stack_All_agency_MaxCorr_0.9_MatchPair_10_MinRegPeriod3.csv", sep = ""), header = TRUE) ### all agency mdata
model.data = model.data[c("monthYear", "levelNAVD88","commonName","data.type")]
model.data.orig = read.csv(paste(mainDir, "data/common.mdata.QA.csv", sep = ""), header = TRUE) ### all agency mdata
...
## DEFINED PARAMETERS ##
# station.list = c("N/A", "NWFWMD1311","SJRWMD01140471","SJRWMD01140472") # subset well selected for greatest correlation with another, need the N/A placeholder in the first value to select for all stations
...
wide.data.orig = dcast(model.data.orig, monthYear~commonName, value.var = "levelNAVD88")
wide.data.orig$monthYear = as.Date(wide.data.orig$monthYear, format = "%Y-%m-%d") # create data for monthYear
wide.data.orig = wide.data.orig[order(wide.data.orig$monthYear),] # order the dataframe
wide.data.orig = wide.data.orig[which((wide.data.orig$monthYear >= start.date) & 
                              (wide.data.orig$monthYear <= end.date)),] # period of record 
...
row.names(wide.data.orig) = wide.data.orig$monthYear
wide.data.orig = wide.data.orig[c("monthYear", as.character(GIS.good.inventory$commonName))] # remove data that does not have inventory data

if(length(station.list) <= 1){
...
for (w in 2:length(station.list)) {
  ...
  GIS.subset.meta =  subset(GIS.good.inventory, latDD > (subset.meta$latDD - 0.5) 
                              ...
                              & lonDD < (subset.meta$lonDD + 0.5))
  location.data = subset(wide.data, select = c("monthYear", as.character(GIS.subset.meta$commonName)))
  location.data.orig = subset(wide.data.orig, select = c("monthYear", as.character(GIS.subset.meta$commonName))) # data that only is within the radius of lat and lon +/- 0.5
  
  for(u in 3:length(colnames(location.data))) # Correlation between subset.label well and all other wells
    {
    check.pairs = nobs(location.data[[subset.label]] + location.data.orig[[u]]) # Number of shared observations
    fillable.pairs = nobs(location.data.orig[[u]]) - check.pairs # Fillable pairs >0 states that there must be data to be filled in.
    end.date.u = max(row.names(na.omit(location.data.orig[u])))
    ind.dates = match(row.names(na.omit(location.data[subset.label])), row.names(na.omit(location.data.orig[u])))  # index of matched dates used in regression
  ...
  if(length(max.cor) == 0){ # if there is no max.cor value, then skip to the next station and fill in spreadsheet with original values
    ...
    stat.model.merge.all.stack = rbind(
      ...
      na.omit(data.frame(monthYear = as.character(location.data$monthYear), 
  ...
  final.reg =lm(location.data[[subset.label]]~location.data.orig[[best.station.name]]) # best correlated well regression model
  ...
  ind1 = data.frame(data.type =as.character(ind), filled ="filled_second", 
  ...
  colnames(stat.model.merge) = c("monthYear", paste(subset.label, "_filled_second", sep = ""))
  ...
  stat.model.merge.all.stack = rbind(
    stat.model.merge.all.stack, 
    na.omit(data.frame(monthYear = as.character(stat.model.merge$monthYear), 
  ...
  subDir = paste("figures/gapFill/", model.name,"_filled_figure_second",subDirParam, sep = "")
  dir.create(paste("H:/Engineering_and_Hydro_Science/Projects/Groundwater_Resource_Assessment/groundwater_levels_NAVD88_Florida_Georgia/groundwater/", subDir, sep = ""), showWarnings = TRUE)
  png(paste(file = "H:/Engineering_and_Hydro_Science/Projects/Groundwater_Resource_Assessment/groundwater_levels_NAVD88_Florida_Georgia/groundwater/",subDir,"/Hydrograph_",subset.label,".png", sep =""),
      width = 1000, height = 650, units= "px", res=100)        
  par(mfrow = c(2,2), mar = c(3,4,3,1))
  
  ## Hydrographs of two stations
  ylimlow = min(c(location.data[[subset.label]],location.data.orig[[best.station.name]]), na.rm = TRUE)
  ylimhigh = max(c(location.data[[subset.label]],location.data.orig[[best.station.name]]), na.rm = TRUE)
  ...
  points(location.data$monthYear,location.data.orig[[best.station.name]], 
  ...
  plot(location.data[[subset.label]]~location.data.orig[[best.station.name]], 
  ...
  plot(location.data$monthYear[!is.na(ind)], 
       ...
       main = paste("Filled second station:", subset.label), ylab = "Level (NAVD88,ft)", xlab = "Date", 
  ...
  legend("topleft", bty = "n", c("Original data", "Filled second data"), col = c("black","red"), 
         lty = c(0, 0),pch=c(20,1), merge = F, border=F)
...
##### PRINT DATA FILES #####
second = subset(stat.model.merge.all.stack, subset = stat.model.merge.all.stack$data.type == "filled_second")
second.fill = rbind(na.omit(model.data), na.omit(second))
first = subset(model.data, subset = model.data$data.type == "filled")
stat.model.merge.all.stack = second.fill

write.csv(sum.stat.table, file = paste(mainDir, "data/Sum_stat_table_second_",model.name,"_MaxCorr_",max.cor.threshold, "_MatchPair_",matched.pairs,"_MinRegPeriod",regression.period.overlap.threshold,".csv", sep="")) # summary stats table of statistical models for filling data
write.csv(stat.model.merge.all.wide, file = paste(mainDir, "data/Filled_stations_wide_second_",model.name,"_MaxCorr_",max.cor.threshold, "_MatchPair_",matched.pairs,"_MinRegPeriod",regression.period.overlap.threshold,".csv", sep="")) # wide format .csv
write.csv(stat.model.merge.all.stack, file = paste(mainDir, "data/Filled_stations_stack_second_",model.name,"_MaxCorr_",max.cor.threshold, "_MatchPair_",matched.pairs,"_MinRegPeriod",regression.period.overlap.threshold,".csv", sep="")) # stacked format .csv